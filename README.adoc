...
Network UPS Tools - это набор программ, предоставляющих общий интерфейс для мониторинга и администрирования оборудования ИБП, PDU и SCD.

В нем используется многоуровневый подход для соединения всех частей. Драйверы предоставляются для широкого спектра оборудования. Они понимают специфический язык каждого устройства и отображают его на уровень совместимости. Это означает, что и дорогой ИБП высокого класса, и простой PDU "power strip", и любое другое устройство питания могут работать прозрачно с единым интерфейсом управления. Эта информация кэшируется сетевым сервером upsd, который затем отвечает на запросы клиентов.

upsd содержит ряд функций контроля доступа, чтобы ограничить возможности клиентов. Только авторизованные узлы могут следить за вашим оборудованием или управлять им по вашему желанию. Поскольку концепция мониторинга по сети встроена в программное обеспечение, вы можете повесить множество систем на один большой ИБП, и все они будут отключаться вместе. 

Вы также можете использовать NUT для включения, выключения или циклического отключения питания узлов центра обработки данных, индивидуально или глобально через розетки PDU. Такие клиенты, как upsmon, проверяют состояние оборудования и выполняют необходимые действия. Наиболее важной задачей является чистое завершение работы операционной системы до того, как ИБП разрядится. Также предусмотрены другие программы для регулярной регистрации информации, мониторинга состояния через веб-браузер и т. д.
